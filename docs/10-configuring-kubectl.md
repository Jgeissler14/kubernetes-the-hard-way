...
You should be able to ping `server.us-central1-f.c.instant-bonfire-425921-s5.internal` based on the `/etc/hosts` DNS entry from a previous lap.

```bash
curl -k --cacert ca.crt \
  https://server.us-central1-f.c.instant-bonfire-425921-s5.internal:6443/version
```
...
```bash
{
  kubectl config set-cluster kubernetes-the-hard-way \
    ...
    --embed-certs=true \
    --server=https://server.us-central1-f.c.instant-bonfire-425921-s5.internal:6443
